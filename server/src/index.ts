...
import { z } from 'zod';
import {
  connectWalletToParticipant,
  createBill,
  createParticipantPaymentDraft,
  getBillOrThrow,
  refreshParticipantPayment,
  submitPaymentForParticipant,
...
app.post('/api/bills/:billId/participants/:participantId/wallet', (request, response) => {
  try {
    ...
    response.json({
      bill: connectWalletToParticipant(
...
app.post('/api/bills/:billId/participants/:participantId/submit-payment', (request, response) => {
  try {
    ...
    response.json({
      bill: submitPaymentForParticipant(
        request.params.billId,
        request.params.participantId,
        payload.txId,
        payload.walletAddress,
